...
/// Animated splash screen inspired by Gallery app.
/// Shows a Prism logo animation while preloading mock data.
...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _loadAndTransition() async {
    // Pre-load all mock data while showing splash
    await Future.wait([
      MockDataService.instance.preloadAll(),
  ...
  Widget build(BuildContext context) {
    final bg = const Color(0xFF060610);
...
class _PrismLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 4 shapes at cardinal positions, rotating
    final shapes = [
      _ShapeInfo(const Color(0xFF818CF8), 0),       // Indigo (top)
      _ShapeInfo(const Color(0xFF34D399), pi / 2),   // Emerald (right)
      _ShapeInfo(const Color(0xFFF97316), pi),        // Orange (bottom)
      _ShapeInfo(const Color(0xFFF43F5E), 3 * pi / 2), // Rose (left)
    ];

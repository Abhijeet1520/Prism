...
> Definitions of terms, abbreviations, and concepts used across the Prism documentation suite.
...
| Term | Definition |
|------|-----------|
| **Adapter Pattern** | Design pattern where each AI provider implements a common interface (`AIProvider`), allowing Prism to swap providers without changing calling code. |
| **AES-256-GCM** | Advanced Encryption Standard with 256-bit keys in Galois/Counter Mode. Authenticated encryption providing both confidentiality and integrity. Used for all data at rest in Prism. |
...
| **Apps Hub** | A primary navigation tab that serves as a launcher for secondary features (Tools, Files, Tasks, Finance, Gateway). |
...
| Term | Definition |
|------|-----------|
| **BIP39** | Bitcoin Improvement Proposal 39 — standard for encoding cryptographic seeds as human-readable word lists. Used for Prism's sync recovery key. |
| **Bottom Sheet** | A Material Design component that slides up from the bottom of the screen. Used in Prism for model selection, file actions, and tool previews. |
| **Brain** | A primary navigation tab providing PARA-method knowledge management. Replaced the direct Files tab in the navigation hierarchy. |

## C

| Term | Definition |
|------|-----------|
| **Chat Completion** | An AI API endpoint that takes a conversation history (list of messages) and returns the next assistant message. Core API pattern used by all Prism providers. |
...
| **CPython** | The reference implementation of Python, written in C. Prism embeds CPython via FFI for local Python execution. |

## D

| Term | Definition |
|------|-----------|
| **dart_eval** | A Dart package that evaluates Dart code at runtime without compilation. Used for local Dart code execution in Prism. |
| **Daytona** | A cloud development environment platform. Supported as a remote code execution backend in Prism. |
| **DEK** | Data Encryption Key — the AES-256 key used to encrypt all user data. Itself encrypted by the KEK. |
| **Diff** | The set of changes between two versions of a file. Prism uses Myers algorithm with word-level refinement. |
...
| **Flutter** | Google's cross-platform UI framework using Dart. The development platform for Prism. |
...
| **GGUF** | GGML Universal Format — the file format for quantized LLM models used by llama.cpp and Ollama. Prism loads .gguf files for on-device inference via `llama_sdk`. |
| **PrismProvider** | Wrapper around LangChain.dart's `BaseChatModel` that adds Prism-specific concerns: credential management, rate limiting, cost tracking, and health checks. |
| **PrismFolder** | A virtual folder organizing PrismFiles. Can have permission overrides that cascade to children. |
| **GoRouter** | A declarative routing package for Flutter. Used for Prism's navigation with deep link support. |
...
| Term | Definition |
|------|-----------|
| **HuggingFace** | An AI platform providing model hosting, downloads, and inference APIs. Prism uses HuggingFace for model downloads (with OAuth) and cloud inference. |
| **Hunk** | See DiffHunk. |
...
| Term | Definition |
|------|-----------|
| **Isar** | A fast, embedded NoSQL database for Flutter/Dart. Version 4.x used as Prism's local storage engine. |
...
| **LangChain.dart** | A Dart port of the LangChain framework providing unified abstractions (`BaseChatModel`, `Runnable`, chains, agents, tools) for working with LLMs. Prism's provider abstraction layer. See [langchain_dart](https://pub.dev/packages/langchain). |
| **LiteRT** | Google's on-device AI inference runtime (formerly TensorFlow Lite). Used for running TFLite models in Prism via platform channels. |
| **llama.cpp** | A C/C++ library for local LLM inference supporting 100+ model architectures with extensive quantization. Prism accesses it via `llama_sdk` (FFI) or Ollama. |
...
| **Moon Design** | A Flutter UI toolkit (v1.1.0) using token-based theming with Dragon Ball-named color tokens (piccolo, goten, gohan, bulma, trunks, beerus, etc.). Prism's UI framework. |
| **Myers Algorithm** | An efficient algorithm for computing the minimum edit distance (diff) between two sequences. Core of Prism's versioning engine. |
...
| Term | Definition |
|------|-----------|
| **Ollama** | A tool for running LLMs locally, wrapping llama.cpp with an easy API and model management. First-class provider in Prism via `langchain_ollama`. Supports LAN discovery for mobile-to-desktop connectivity. |
...
| Term | Definition |
|------|-----------|
| **PARA Method** | A knowledge organization system (Projects, Areas, Resources, Archive) used by Prism's Brain module for structuring user knowledge. |
| **PBKDF2** | Password-Based Key Derivation Function 2 — used to derive encryption keys from user passphrases. 600,000 iterations for Prism sync keys. |
...
| Term | Definition |
|------|-----------|
| **QuickJS** | A small, embeddable JavaScript engine. Used for sandboxed JavaScript/TypeScript execution in Prism. |
...
| **Riverpod** | A reactive state management framework for Flutter. Version 2.x used in Prism for compile-safe dependency injection and state management. |
...
| **Supabase** | An open-source Firebase alternative providing auth, real-time database, storage, and edge functions. Chosen for Prism's cloud sync backend (proven in Maid). |
...
| **TLS** | Transport Layer Security — cryptographic protocol for secure network communication. Prism requires TLS 1.3 (or 1.2 minimum). |
...
| **Virtual Filesystem** | Prism's file storage system, implemented in Isar (not the device OS filesystem). Provides files, folders, encryption, and versioning. |

## W

| Term | Definition |
|------|-----------|
| **WCAG AA** | Web Content Accessibility Guidelines level AA — the target accessibility standard for Prism, covering contrast, screen readers, and keyboard navigation. |
